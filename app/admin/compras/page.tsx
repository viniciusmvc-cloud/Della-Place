...
export default function ComprasPage() {
  ...
  async function reload() {
    setLoading(true);
    try {
      const [prods, all, carry, pending, ords] = await Promise.all([
        fetchProducts().catch(() => []),
  ...
  // Histórico agrupado por ciclo (production_date) — só ciclos passados
  const pastCycles = useMemo(() => {
    const today = new Date().toISOString().slice(0, 10);
    const map = new Map<
      string,
      { items: Purchase[]; totalCost: number }
    >();
    allPurchases
        ...
        <section className="rounded-xl border border-primary-100 bg-white">
          <header className="border-b border-primary-100 p-4">
            ...
            <p className="mt-1 text-xs text-primary-500/60">
              Compras consumidas em ciclos já encerrados. Custo refletido no BI e Relatórios.
            </p>
          </header>
          <ul className="divide-y divide-primary-100">
            {pastCycles.map(([date, bucket]) => {
              const pizzas = pizzasByDate.get(date) ?? 0;
